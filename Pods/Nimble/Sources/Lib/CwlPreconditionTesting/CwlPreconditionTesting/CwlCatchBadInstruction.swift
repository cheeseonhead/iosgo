...
import Foundation
    ...
    extension execTypesCountTuple
    {
        mutating func pointer<R>(in block: (UnsafeMutablePointer<T>) -> R) -> R
        {
            ...
            { p -> R in
                return p.withMemoryRebound(to: T.self, capacity: EXC_TYPES_COUNT)
                { ptr -> R in
                    return block(ptr)
                }
            }
    ...
    extension request_mach_exception_raise_t
    {
        mutating func withMsgHeaderPointer<R>(in block: (UnsafeMutablePointer<mach_msg_header_t>) -> R) -> R
        {
            return withUnsafeMutablePointer(to: &self)
            { p -> R in
                return p.withMemoryRebound(to: mach_msg_header_t.self, capacity: 1)
                { ptr -> R in
                    return block(ptr)
                }
            }
    ...
    private struct MachContext
    {
        ...
        {
            return masks.pointer
            { masksPtr in
                return ports.pointer
                { portsPtr in
                    return behaviors.pointer
                    { behaviorsPtr in
                        return flavors.pointer
                        { flavorsPtr in
        ...
        { do
            {
                ...
                if !handledfirstException
                {
                    ...
                    guard request.withMsgHeaderPointer(in: { requestPtr in reply.withMsgHeaderPointer
                        { replyPtr in
    ...
    public func catchBadInstruction(in block: () -> Void) -> BadInstructionException? {
        ...
        do
        {
            ...
            { context.withUnsafeMutablePointers
                { masksPtr, portsPtr, behaviorsPtr, flavorsPtr in
            ...
            try withUnsafeMutablePointer(to: &context)
            { c throws in
                // 4. Create the thread
